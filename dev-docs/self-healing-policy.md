...
- proposal 在展示给用户前，必须先经过二次审计；若引入新的 repair 风险，直接阻断展示并提示 `该修复方案引入了新的风险，已被系统自动拦截`。
- proposal 在展示给用户前，必须先经过快速语法检查；若语法不合法，直接阻断展示并提示 `该修复方案未通过语法检查，已被系统自动拦截`。
- proposal 展示通过时，工作台至少显式显示三行：`✅ 二次审计通过`、`✅ 语法检查通过`、`⚠️ 逻辑变更提示 ...`。
